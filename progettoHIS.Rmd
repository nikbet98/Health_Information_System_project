...
```{r}
df$Aprx_altitude <- df$Quota %>% {case_when(. > 0 & . < 50 ~ 50,
                                   . > 50 & . < 100 ~ 100,
                                   . > 100 & . < 150 ~ 150,
                                   . > 150 & . < 200 ~ 200,
                                   . > 200 & . < 300 ~ 300,
...
altitudes_average <- df %>% 
  subset(select = c(NomeTipoSensore,Valore,Aprx_altitude)) %>% 
  pivot_wider(names_from = NomeTipoSensore, values_from = Valore, values_fn = mean)
...
for (c in pollutans){
   p <- ggplot(map_with_value) + 
     geom_sf(aes(fill = map_with_value[[c]])) + 
     scale_fill_gradient(low = "#a7b4e8",high = "#2e408c", name = c) + 
     ggtitle(c) + 
     theme(plot.title = element_text(hjust = 0.5))
   
   path <- paste("./image/map",gsub(" ","",c),".png",sep="")
   ggsave(path, plot=p, dpi=300)
}
