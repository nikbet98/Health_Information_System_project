...
This dataset consist in more of 17 million records, each of it represents a sensor reading, each reading is described by 4 values: idSensore, Data, Valore, Stato, idOperatore.
...
This dataset does not indicate us directly what was the pollutants associated with the readings, for this will be necessary the next dataset that will be utilized to associate the pollutant to the sensor.
...
str(df_stations)
```
From this dataset we will take some important information like: the pollutants analized from the sensor, the geographical location of it, not only his latitude and longitude, but also the city, the province and the altitude; another useful information will be the unit of measurement of the value read by the sensor.


## Dataset temperature medie milano extra
...
Beside the dataset we will take in consideration also the guidelines established by the WHO fro the target value and the safety value for the pollutants, this can be found directly from the publication <https://www.who.int/publications/i/item/9789240034228>, where the old guidelines (from 15 years ago) has been update thanks to the better knowledge on the effect from the exposure of these pollutants on the human system for prolunged time.
...
```{r}
pollutans <- c("Biossido di Azoto", "Monossido di Azoto", "Ossidi di Azoto", "Biossido di Zolfo", "Monossido di Carbonio", "Ozono", "PM10 (SM2005)", "Particelle sospese PM2.5", "Benzene")
```
...
<!-- PM2.5 recommended leve and target values -->
```{r}
...
```{r}
ggplot(annual_averages) +
  aes(x=Year, y=`PM10 (SM2005)`, group = 1) +
  geom_line() +
...
```{r}
averages <- df %>% 
  filter(NomeTipoSensore == "PM10 (SM2005)") %>%
  group_by(Dates) %>% 
  summarise(mean = mean(Valore))
...
<!-- CO recommended leve and target values -->
```{r}
averages <- df %>% 
  filter(NomeTipoSensore == "Monossido di Carbonio") %>%
  group_by(Dates) %>% 
  summarise(mean = mean(Valore))
...
```{r}
ggplot(annual_averages) +
  aes(x=Year, y=`Biossido di Azoto`, group = 1) +
  geom_line() +
  geom_hline(aes(yintercept=120, linetype="Interim target 1"), color = "red", size = 1) +
  geom_hline(aes(yintercept=50, linetype="Interim target 2"), color = "blue", size = 1) +
  geom_hline(aes(yintercept=25, linetype="AQG level"), color = "green", size = 1) +
...
```{r}
averages <- df %>% 
  filter(NomeTipoSensore == "Biossido di Azoto") %>%
  group_by(Dates) %>% 
  summarise(mean = mean(Valore))
...
ggplot(averages) +
  ...
  geom_hline(aes(yintercept=10, linetype="AQC level"), color = "green", size = 1) +
...
```{r}
averages <- df %>% 
  filter(NomeTipoSensore == "Biossido di Zolfo") %>%
  group_by(Dates) %>% 
  summarise(mean = mean(Valore))
